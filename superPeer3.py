...
host = 'localhost'
port = params.peer20_port
#List of neighbours
port_ps = params.peer20_conn
port2 = 7010
...
def handlePeer(peerServer):
    global queryStat

    while True:
        try:
            dataIn = pickle.loads(peerServer.recv(1024))
        ...
        if queryStat and dataSplit[0] == "QUERYHIT":
            peerServer.sendall(pickle.dumps("OK"))
        ...
        if dataSplit[0] == "QUERY":
            if not handleQUERY(peerServer, dataIn):
                return True
...
def main():
    ...
    while True:
        ...
        threading.Thread(target=peerServer, args=(client,)).start()
