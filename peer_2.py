...
host = 'localhost'
port_srv = params.peer10_port

# port for other peer to connect to it
port_client = params.peer2_port

global shared_dirPath
# relative path (to project) to shared directory
shared_dirPath = params.peer2_path

global peerID
# ID of this client
peerID = params.peer2_id
...
def upload_server():
    ...
    print("->Upload server is now running at port", port_client)
    while True:
        try:
            s_up.listen(5)
...
def main():
    ...
    try:
        s.connect((host, port_srv))
        print("->Connected to the server at port", port_srv)
    except:
    ...
    print("Do you want to lookup for any file ??? (Y/N)")
    if input() == "Y":
